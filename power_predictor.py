...
class EnergyPredictor(object):
    def __init__(self, feature_norm, lat_norm, ckpt_path, lat_dataset_path='./energy_dataset/lat.tmp', feature_dim=10, hidden_dim=400, hidden_layer_num=3, train_steps=5000, bsz=128, lr=1e-5):
    ...
    def train(self):
        ...
        with torch.no_grad():
            ...
            prediction = self.model(sample_x_tensor).squeeze()
            loss = self.criterion(prediction, sample_y_tensor)
            print(f"Predicted energy: {prediction}")
            print(f"Real energy: {self.test_y}")
            print(f"Loss: {loss}")
...
if __name__=='__main__':
    ...
    parser.add_argument('--lat-dataset-path', type=str, default='./energy_dataset/lat.tmp', help='the path to read energy dataset')
    ...
    parser.add_argument('--lat-norm', type=float, default=200, help='normalizing factor for energy')
    parser.add_argument('--feature-dim', type=int, default=10, help='dimension of feature vector')
    parser.add_argument('--hidden-dim', type=int, default=400, help='hidden dimension of FC layers in energy predictor')
    parser.add_argument('--hidden-layer-num', type=int, default=3, help='number of FC layers')
    parser.add_argument('--ckpt-path', type=str, default='energy_dataset/ckpts/tmp.pt', help='path to save energy predictor weights')
    parser.add_argument('--train-steps', type=int, default=5000, help='energy predictor training steps')
    parser.add_argument('--bsz', type=int, default=128, help='energy predictor training batch size')
    parser.add_argument('--lr', type=float, default=1e-5, help='energy predictor training learning rate')

    args = parser.parse_args()
    print(args)

    predictor = EnergyPredictor(lat_dataset_path=args.lat_dataset_path,
    ...
    predictor.split()
    predictor.train()
    print('Energy predictor training finished')
    ...
    print(f'Example energy: {predict}')
