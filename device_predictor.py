...
class Net(nn.Module):
    def __init__(self, feature_dim, hidden_dim, hidden_layer_num, output_dim):
...
class DevicePredictor(object):
    def __init__(self, feature_norm, lat_norm, ckpt_path, lat_dataset_path='./device_dataset/lat.tmp', feature_dim=15, hidden_dim=400, hidden_layer_num=3, output_dim=4, train_steps=5000, bsz=32, lr=1e-5):
        ...
        self.model = Net(self.feature_dim, self.hidden_dim, self.hidden_layer_num, self.output_dim)
    ...
    def predict_lat(self, config):
        with torch.no_grad():
            ...
            features += [32,32,256,4096,1]
            features_norm = np.array(features) / self.feature_norm

            prediction = self.model(torch.Tensor(features_norm)) * self.lat_norm
    ...
    def read_dataset(self):
        ...
        with open(self.dataset_path, 'r') as fid:
            ...
            for line in fid:
                ...
                lats_eval = list(map(eval, lats))
                lats_norm = np.array(lats_eval) / self.lat_norm
                lats_all.append(lats_norm)
...
if __name__=='__main__':
    ...
    parser.add_argument('--lat-dataset-path', type=str, default='./device_dataset/lat.tmp', help='the path to read latency dataset')
    parser.add_argument('--feature-norm', type=float, nargs='+', default=[640, 6, 2048, 6, 640, 6, 2048, 6, 6, 2, 64, 64, 512, 8192, 1], help='normalizing factor for each feature')
    parser.add_argument('--lat-norm', type=float, default=10000000, help='normalizing factor for cache misses') #TODO make it a feature-norm like array?
    parser.add_argument('--feature-dim', type=int, default=15, help='dimension of feature vector')
    ...
    parser.add_argument('--output-dim', type=int, default=4, help='dimension of output vector')
    parser.add_argument('--ckpt-path', type=str, default='device_dataset/ckpts/tmp.pt', help='path to save latency predictor weights')
    ...
    predictor = DevicePredictor(lat_dataset_path=args.lat_dataset_path,
